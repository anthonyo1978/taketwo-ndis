...
interface TransactionsTableProps {
  ...
  /** When true, shows a green left-border accent on each row to distinguish income */
  showIncomeAccent?: boolean
}
...
export function TransactionsTable({ filters, onCreateTransaction, refreshTrigger, showIncomeAccent = false }: TransactionsTableProps) {
        ...
        <table className="w-full">
          <thead className="bg-gray-50">
            {table.getHeaderGroups().map(headerGroup => (
              <tr key={headerGroup.id}>
                {showIncomeAccent && <th className="w-1 px-0 py-3"></th>}
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {enhancedData.length === 0 ? (
              <tr>
                <td colSpan={columns.length + (showIncomeAccent ? 1 : 0)} className="px-6 py-12 text-center">
                ...
                <tr key={row.id} className="hover:bg-gray-50">
                  {showIncomeAccent && (
                    <td className="w-1 px-0 py-0">
                      <div className="w-1 h-full min-h-[48px] bg-green-500 rounded-r" />
                    </td>
                  )}
