...
export function ResidentForm({ houseId, mode = "house-context", open, onClose, onSuccess }: ResidentFormProps) {
  ...
  const handleFormSubmit = async (data: ResidentCreateSchemaType) => {
    ...
    try {
      ...
      // In standalone mode, house assignment is optional (Prospect residents)
      if (mode === "house-context" && !targetHouseId) {
      ...
      // For standalone mode, add houseId to formData only if one is selected
      if (mode === "standalone" && targetHouseId) {
    ...
    <Dialog open={open} onClose={handleClose}>
      <DialogContent>
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
          ...
          {/* House Selection (Standalone Mode Only) - Optional for Prospect residents */}
          {mode === "standalone" && (
            <div className="space-y-4">
              <h4 className="text-md font-medium text-gray-900">House Assignment (Optional)</h4>
              <p className="text-sm text-gray-600">
                Residents can be created in Prospect mode without house assignment. 
                House assignment can be done later in the Houses module.
              </p>
